...
void update(Vertex* v) {
    ...
    auto sum_of = [](Vertex* v) { if (v) { return v->sum; } else { return static_cast<long long>(0); } };
    v->sum = v->key + sum_of(v->left) + sum_of(v->right);
    if (v->left != NULL) {
        v->left->parent = v;
    }
    if (v->right != NULL) {
...
void erase(int x) {
    ...
    auto result = find(root, x);
    if (result && result->key == x)
    {
        // Splay is done during find
        //splay(root, result);
        auto left_child = result->left;
        auto right_child = result->right;
        if (left_child && right_child)
        {
            left_child->parent = nullptr;
            right_child->parent = nullptr;
            root = merge(left_child, right_child);
            delete result;
        }
        else if (left_child)
        {
...
long long sum(int from, int to) {
    //std::cout << "sum " << from << " to " << to << std::endl;
    Vertex* left = NULL;
    Vertex* middle = NULL;
    Vertex* right = NULL;
    ...
    auto merge1 = merge(left, middle);
    root = merge(merge1, right);
